...
var projectile_database = {
	"single":{
		...
		damage = 1,
		duration = 100,
		sprite_region_rect=Rect2(5,2,2,1),
		collision_extents=Vector2(1,0.5)
	},
	"heavy":{
		velocity = 250,
		damage = 5,
	...
	"orb":{
		...
		damage = 6,
		duration = 320,
		sprite_region_rect=Rect2(17,5,3,5),
		collision_extents=Vector2(1.5,2.5)
	}
}
...
func _on_Projectiles_area_entered(area): #Collisions with area 2ds such as shields, area_is_active has to be true to collide
