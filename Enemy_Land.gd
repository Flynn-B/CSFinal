...
var enemy = {
		...
		"rand_extra_health" : 0, #Extra health random ontop of health
...
var enemy_database = {
	"crawler" : {
		...
		"user":"Land_Enemy",
		#Health based variables
		"rand_extra_health" : ceil(rand_range(0,15)), #Extra health random ontop of health
...
func _ready():
	...
	enemy.health+=enemy.rand_extra_health
	enemy.max_health+=enemy.rand_extra_health
	if enemy.weapon_type=="None":
		$Weapon_Land.hide()
	else:
		$Weapon_Land.weapon_type=$Weapon_Land.weapon_types[floor(rand_range(0,3))]
		$Weapon_Land.weapon=$Weapon_Land.weapon_database[$Weapon_Land.weapon_type]
		$Weapon_Land/Sprite.region_rect=$Weapon_Land.weapon.sprite_rect
...
func get_movement():
	if active: #If onscreen
		if get_parent().get_node("Player_Land").player_dead:
			queue_free()
